...
void drawCS()
{
	//Streckt WireCube entland der x-Achse
	glm::mat4 Save = Model;
	Model = glm::scale(Save, glm::vec3(2.0, 0.01, 0.01));
	sendMVP();
	drawWireCube();
#ifdef UEBUNG10
	// Strecke einen WireCube entlang der y-Achse
	Model = glm::scale(Save, glm::vec3(0.01, 2.0, 0.01));

	// für jede neue Trandsformation muss sendMPV aufgerufen werden , oder anderes audgedrückt für jeden Balken
	sendMVP();
	//Zeichnen des Objectes nicht vergessen!!!
	drawWireCube();

	//Strecke einen WireCube eintlang der z-Achse
	Model = glm::scale(Save, glm::vec3(0.01, 0.01, 2.0));
	sendMVP();
	drawWireCube();


	//Nicht vergessen am ende die zwischen gespeicherte Modek Transformation zurück zu speicheren (Model = Save)
#endif
...
void drawSeg(GLfloat height) {
	//Transformationsmatrize zwischenspeichern
	glm::mat4 Save = Model;
	//Translation des Models abhängig von der Höhe (height/2)
	Model = glm::translate(Model, glm::vec3(0.0, height/2, 0.0));

	//Scaling des Models abhängig von der Höhr , in X und Z schmaler (hright/6)
	Model = glm::scale(Model, glm::vec3(height/6, height/2, height/6));

	//Transformation zur Grafikkarte senden
	sendMVP();

	//Sphere zeichnen , Parameter 10,10
	drawSphere(10, 10);

	//Transformationsmatrize zurück speichern
	Model = Save;
...

{
	...
	#ifdef UEBUNG2
		#ifdef UEBUNG3
		Model = glm::rotate(Model, angelX, glm::vec3(1, 0, 0));
		Model = glm::rotate(Model, angelY, glm::vec3(0, 1, 0));
		Model = glm::rotate(Model, angelZ, glm::vec3(0, 0, 1));
...
#else
		Model = glm::rotate(Model, 30.0f, glm::vec3(-1, 0, 0));
#endif
...
#ifdef UEBUNG14
	Model = glm::rotate(Model, (GLfloat)90.0, glm::vec3(-1.0, 0, 0));
...
#ifdef UEBUNG11
#ifdef UEBUNG13
		//Rotieren des Model mit Angel0 entlang der Y-Richtung
		Model = glm::rotate(Model, angel0, glm::vec3(0, 1, 0));
		//Rotieren des Model mit Angel0 entlang der Z-Richtung

		Model = glm::rotate(Model, angel1, glm::vec3(0, 0, 1));
#endif
		drawSeg(1.0);
#ifdef UEBUNG12

		// Transliere Model um 1 
		Model = glm::translate(Model, glm::vec3(0.0,  1, 0.0));
#ifdef UEBUNG13
		//Rotiere das Model mit Angel2 entlang der Z-Richtung
		
		Model = glm::rotate(Model, angel2, glm::vec3(0, 0, 1));


#endif
		//Zeichne Segment mit 0,8

		drawSeg(0.8);
		//Transliere Model um 0,8

		Model = glm::translate(Model, glm::vec3(0.0, 0.8, 0.0));
		//Zeichne Segment ,it 0,6
		Model = glm::rotate(Model, angel3, glm::vec3(0, 0, 1));
